	...
	var methods =
	{
		init : function(options)
		{
    	    ...
    	    {	
				...
				var fieldSet  	= $('<fieldset />').append('<legend class="ax-legend">' + languageArray["selectfiles"] + '</legend>').appendTo($this);
				var browse_c  	= $('<a class="ax-browse-c ax-button" title="' + languageArray["addfiles"] + '" />').appendTo(fieldSet);//Browse container
				browse_c.append('<span class="ax-plus-icon ax-icon"></span> <span>' + languageArray["addfiles"] + '</span>');
				
				var browseFiles = $('<input type="file" class="ax-browse" name="ax-files[]" />').attr('multiple',isAjaxUpload).appendTo(browse_c);
				
			    var uploadFiles = $('<a class="ax-upload-all ax-button" title="' + languageArray["uploadfiles"] + '" />').appendTo(fieldSet);
			    uploadFiles.append('<span class="ax-upload-icon ax-icon"></span> <span>' + languageArray["uploadfiles"] + '</span>');
			    
			    var removeFiles = $('<a class="ax-clear ax-button" title="' + languageArray["removeall"] + '" />').appendTo(fieldSet);
			    removeFiles.append('<span class="ax-clear-icon ax-icon"></span> <span>' + languageArray["removeall"] + '</span>');
				...
				emptyQueueAction = (function(settingsObj)
				{
					return function() { clearQueue(settingsObj); };
				})(settings);
